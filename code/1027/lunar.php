<?php
/**
 * 这是一个实用的PHP日期时间操作类，里面包括了公历-农历转换、转换成中文日期格式、计算农历相隔天数、根据阴历年获取生肖、
 * 获取阴历月份的天数、获取农历每年的天数、获取闰月、计算阴历日期与正月初一相隔的天数、
 * 计算2个公历（阳历）日期之间的天数、根据距离正月初一的天数计算阴历日期、获取天干地支纪年等，
 * PHP日期操作类：Lunar.class.php代码如下：
 * 
 */
class Lunar {
	var $MIN_YEAR = 1891;
...
$date = $lunar->convertLunarToSolar(2013,2,12); //公历转农历
$date = $lunar->getYearZodiac(2013,2,12); //根据阴历年获取生肖
$date = $lunar->isLeapYear(2013,2,12); //判断闰年
//其它功能也是这种调用方法
print_r($date);
//如果是农历转公历，代码如下：
$date = $lunar->convertLunarToSolar(2013,2,12); //农历转公历
print_r($date);

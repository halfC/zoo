	...
	fill : function() {
		...
		for(var i = 0; i < me.config.nMonths; i++) {
			...
			while(prevMonth.valueOf() < nextMonth.valueOf()) {
				...
				//alert(me.selectedDateAry);
				if(in_array(prevMonth.valueOf(),me.config.noDay)){
					clsName += ' disabled';
					//alert(prevMonth.valueOf());
					notClick = true;
				}
				if(!notShow) {
					if(notClick){
					...
					if($kit.hsCls(target, 'day') && !$kit.hsCls(target, 'disabled')) {
						...
						if(me.date && me.date.valueOf() == newDate.valueOf()) {
							...
							if(e.shiftKey && me.config.canMultipleChoose && me.selectedDateAry && me.selectedDateAry.length >= 1) {
								me.addValue(true);
							} else if( me.config.canMultipleChoose && me.selectedDateAry && me.selectedDateAry.length >= 1) {//e.ctrlKey && 默认多选
								me.addValue();
							} else {

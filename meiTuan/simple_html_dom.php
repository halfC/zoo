...
function file_get_html_c($url,$snoopy, $use_include_path = false, $context=null, $offset = -1, $maxLen=-1, $lowercase = true, $forceTagsClosed=true, $target_charset = DEFAULT_TARGET_CHARSET, $stripRN=true, $defaultBRText=DEFAULT_BR_TEXT, $defaultSpanText=DEFAULT_SPAN_TEXT)
{
    // We DO force the tags to be terminated.
    $dom = new simple_html_dom(null, $lowercase, $forceTagsClosed, $target_charset, $stripRN, $defaultBRText, $defaultSpanText);
    // For sourceforge users: uncomment the next line and comment the retreive_url_contents line 2 lines down if it is not already done.
    //$contents = file_get_contents($url, $use_include_path, $context, $offset);
	$contents = snoopyFetch($url,$snoopy);
	//var_dump($contents);
    // Paperg - use our own mechanism for getting the contents as we want to control the timeout.
    //$contents = retrieve_url_contents($url);
    if ( empty($contents) || strlen($contents) > MAX_FILE_SIZE || $contents == false)
...
function snoopyFetch($url,$snoopy){
        ...
        $snoopy->cookies['domain'] = '.sogou.com';
        $snoopy->cookies['expires'] = 'Thu, 01-Dec-1994 16:00:00 GMT';
        $snoopy->cookies['black_passportid'] = '1';
        $snoopy->cookies['CXID'] = 'E2172EF7B7926740D90FAB788A49668D';
        $snoopy->cookies['SUID'] = '5E95F472516C860A567A0506000A3F91';
        $snoopy->cookies['weixinIndexVisited'] = '1';
        $snoopy->cookies['SNUID'] = '36633D300E0B23A59425E4280E7E8E9C';
        $snoopy->cookies['sct'] = '4';
        $snoopy->cookies['ABTEST'] = '2|1455592123|v1';

        $snoopy->cookies['usid'] = 'bnK2rgjNaJM6cS0h';
        $snoopy->cookies['ad'] = 'MZllllllll2Qe7$DlllllVbphrZllllltsnKvZllll9lllll9Cxlw@@@@@@@@@@@';
        //$snoopy->Transfer-Encoding = 'chunked';
